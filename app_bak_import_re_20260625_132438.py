...
from core.utils import load_first_page_image, crop_relative, draw_roi_overlay, split_fixed_slots, trim_to_ink_bbox, slot_ink_ratio
...
""")
    ...
    st.caption("MVP anti-Gowex: auto-trim de tinta + slots validats. No guardem slots buits.")
    ...
    if pdir:
        ...
        st.markdown("#### Configuració de segmentació")
        ...
        ink_ratios = [slot_ink_ratio(s, threshold=trim_threshold) for s in slots]

        # Slot buit: llindar conservador. Si un slot té menys del 0.5% de tinta, sospitós.
        blank_slots = [i + 1 for i, r in enumerate(ink_ratios) if r < 0.005]
        ...
        with c_left:
            ...
            st.markdown("#### ROI després d'auto-trim")
            st.image(crop, caption=f"ROI neta per segmentar · {crop.size[0]} x {crop.size[1]} px", use_container_width=False)

            if use_auto_trim:
                st.success("Auto-trim aplicat: els marges blancs no entren al split.")
            else:
                st.warning("Auto-trim desactivat: el split pot caure en marges blancs.")

            st.markdown("#### Slots detectats")
            slot_cols = st.columns(min(fixed_length, 10))
            for i, slot in enumerate(slots):
                with slot_cols[i % min(fixed_length, 10)]:
                    st.image(slot, caption=f"pos {i+1} · ink {ink_ratios[i]*100:.1f}%", use_container_width=True)

            if blank_slots:
                st.error(f"Slots sospitosament buits: {blank_slots}. No guardis així.")
            else:
                st.success("Cap slot buit detectat. Segmentació usable.")
        ...
        with c_right:
            ...
            force_save_blank = st.checkbox("Forçar guardar encara que hi hagi slots buits (NO recomanat)", value=False)

            can_save = valid_truth and (not blank_slots or force_save_blank)

            if blank_slots and not force_save_blank:
                st.error("Guardar bloquejat: hi ha slots buits. Ajusta ROI o auto-trim.")

            if st.button("💾 Guardar slots etiquetats", type="primary", disabled=not can_save, use_container_width=True):
                already = False
                ...
                if already:
                    ...
                    for i, (slot, ch, ink) in enumerate(zip(slots, truth, ink_ratios), start=1):
